...
#import "YFFullScreenAdView.h"
#import "SDWebImageManager.h"
#import "UIImage+FitInSize.h"
@interface YFFullScreenAdView ()
...
@implementation YFFullScreenAdView
...
- (UIButton *)skipButton
{
    if (!_skipButton) {
        ...
        _skipButton.frame = CGRectMake(MAIN_WIDTH - 70, 30, 60, 30);
        ...
        __weak YFFullScreenAdView *weakSelf = self;
        [_skipButton addTarget:weakSelf action:@selector(skipAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _skipButton;
}

- (UIImageView *)YFSeocndAdImageView
{
    if (!_YFSeocndAdImageView) {
        _YFSeocndAdImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, MAIN_WIDTH, MAIN_HEIGHT/3*2)];
        _YFSeocndAdImageView.userInteractionEnabled = YES;
        __weak YFFullScreenAdView *weakSelf = self;
        [weakSelf addSubview:_YFSeocndAdImageView];
    }
    return _YFSeocndAdImageView;
}
...
{
    if (!_timerView) {
        self.timerView = [[UIView alloc] initWithFrame:CGRectMake(MAIN_WIDTH - 62, 32, 40, 40)];
...
/** 获取广告图 */
- (void)reloadAdImageWithUrl:(NSString *)urlStr  WithProvice:(NSString *)provice
{
    if (urlStr.length <= 0) {
    ...
    NSString* tempprovice = [[NSUserDefaults standardUserDefaults] objectForKey: @"provice"]; //

    if ([tempprovice isEqualToString:@""] || tempprovice == nil) {
        tempprovice = @"全国";
          [[NSUserDefaults standardUserDefaults] setObject:tempprovice forKey:@"provice"];
    }
...
- (void)adImageShowWithImage:(UIImage *)image
{
    if (_flag) return;
    if (_timerWait) dispatch_source_cancel(_timerWait);
    self.image = [UIImage imageNamed:@"default_adv"];
    self.userInteractionEnabled = YES;
    self.contentMode = UIViewContentModeScaleAspectFit;
    self.clipsToBounds = YES;
    self.backgroundColor = [UIColor whiteColor];
    UIImage * tempImage = [UIImage imageCompressForWidth:image targetWidth:MAIN_WIDTH];
    self.YFSeocndAdImageView.image = tempImage;
    self.YFSeocndAdImageView.contentMode = UIViewContentModeScaleAspectFill;
    ...
    [self.YFSeocndAdImageView addGestureRecognizer:tap2];
...
- (void)dismiss
{
    ...
    [UIView animateWithDuration:0.5 delay:0.3 options:UIViewAnimationOptionCurveEaseOut animations:^{
        self.YFSeocndAdImageView.transform = CGAffineTransformMakeScale(1.2, 1.2);
        self.YFSeocndAdImageView.alpha = 0.0;
...
- (void)tapAction
{
    ...
    [self dismiss];
    self.adImageTapBlock(self.pushUrl);
}
