...
#import "YFGuideView.h"

typedef NS_ENUM(NSInteger, YFGuideMaskItemRegion)
{
    /// 左上方
    YFGuideMaskItemRegionLeftTop = 0,
    
    /// 左下方
    YFGuideMaskItemRegionLeftBottom,
    
    /// 右上方
    YFGuideMaskItemRegionRightTop,
    
    /// 右下方
    YFGuideMaskItemRegionRightBottom
};

@interface YFGuideView()
/** 👀 蒙板 👀 */
...
{
   YFGuideView *guideView = [[YFGuideView alloc] initWithFrame:CGRectZero];
...
- (void)congifureItemsFrame
{
    ...
    /// 获取 item 的 方位
    YFGuideMaskItemRegion itemRegion = [self fetchVisualRegion];
    
    switch (itemRegion)
    {
        case YFGuideMaskItemRegionLeftTop:
        {
            /// 左上
        ...
        case YFGuideMaskItemRegionRightTop:
        {
            /// 右上
            arrowRect = CGRectMake(CGRectGetMidX([self fetchVisualFrame]) - imgSize.width * 0.5,
                                   CGRectGetMaxY([self fetchVisualFrame]) + space,
        ...
        case YFGuideMaskItemRegionLeftBottom:
        {
            /// 左下
            transform = CGAffineTransformMakeScale(-1, -1);
            arrowRect = CGRectMake(CGRectGetMidX([self fetchVisualFrame]) - imgSize.width * 0.5,
        ...
        case YFGuideMaskItemRegionRightBottom:
        {
            /// 右下
            transform = CGAffineTransformMakeScale(1, -1);
            arrowRect = CGRectMake(CGRectGetMidX([self fetchVisualFrame]) - imgSize.width * 0.5,
 ...
 *  获取可见区域的方位
 */
- (YFGuideMaskItemRegion)fetchVisualRegion
{
    /// 可见区域的中心坐标
    ...
    {
        /// 当前显示的视图在左上角
        return YFGuideMaskItemRegionLeftTop;
    }
    ...
    {
        /// 当前显示的视图在右上角
        return YFGuideMaskItemRegionRightTop;
    }
    ...
    {
        /// 当前显示的视图在左下角
        return YFGuideMaskItemRegionLeftBottom;
    }
    
    /// 当前显示的视图在右下角
    return YFGuideMaskItemRegionRightBottom;
}
...
/**
 例：
 @interface HomeViewController ()<YFGuideViewDataSource, YFGuideViewLayout>
 @property (strong,nonatomic) NSArray<UIView *> *viewArr;
 //=========
 
 - (void)show
 {
 YFGuideView *maskView = [[YFGuideView alloc] initWithDatasource:self];
